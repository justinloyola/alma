...
class FileUploadManager:
    ...
    def __init__(self, allowed_types: Optional[Dict[str, str]] = None, max_size: int = 5 * 1024 * 1024):
    ...
    async def validate_file(self, file: UploadFile) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        while True:
            ...
            if file_size > self.max_size:
                return False, f"File size exceeds the maximum allowed size of {self.max_size / (1024 * 1024):.1f}MB", metadata
        ...
        if not file_extension:
            return False, f"Unsupported file type. Allowed types: {', '.join(self.allowed_types.values())}", metadata
    ...
    async def save_file(
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid storage type: {storage_type}. Must be one of: {', '.join(e.value for e in StorageType)}"
                )
            ...
            result = {
                'success': True,
                'download_url': storage.get_file_url(lead)
            }
            ...
            db.rollback()
            logger.error(f"Error saving file: {str(e)}", exc_info=True, extra={
                'action': 'save_file',
                'lead_id': lead_id,
                'error': str(e),
